...
xMD5Update(struct xMD5Context *ctx, byte const *buf, int len)
{
	...
	if ((unsigned)t > len) {
		memcpy((byte *)ctx->in + 64 - (unsigned)t, buf, (size_t)len);	/* len >= 0 */
		return;
	}
	...
	memcpy(ctx->in, buf, (size_t)len);	/* len in [0,63] here */
}
...
xMD5Final(byte digest[16], struct xMD5Context *ctx)
{
	...
	*p++ = 0x80;
	...
	if (count < 0) {	/* Padding forces an extra block */
		memset(p, '\0', (size_t)(count+8));	/* count+8 >= 0 */
		byteSwap(ctx->in, 16);
		xMD5Transform(ctx->buf, ctx->in);
		p = (byte *)ctx->in;
		count = 56;
	...
	memset(ctx, '\0', sizeof(*ctx));	/* wipe the struct, not the pointer */
}
 ...
 *  addressable RNG layer so a derived stream is stable across a turn
 *  regardless of how many global draws have happened.
 */
...
int md5_int(int a, int b, int c, int d)
{
	...
	return (int) buf[0];	/* word32 -> int, modular */
}
